...
**5. Taglines are still German in English mode.** Everything else now
translates. Ingredients and glassware are plain nouns, so `data/terms.js`
carries English for all of them, brand names pass through untouched, and the
build fails loudly if a new export introduces a term with no English.

The taglines and bartender notes are different: they are the house voice, and
machine-translating them would put words in the bar's mouth. So they stay
German until someone writes them.

**The code is already waiting for them.** Add `tagline_en` and, optionally,
`bartender_note_en` to each drink in `data/cocktails.json`, rebuild, and they
appear — no code change needed. The build reports how many drinks still lack
one on every run.
...
data/terms.js           English for ingredients and glassware
